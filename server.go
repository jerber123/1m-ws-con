	...
	for {
		...
		for _, conn := range connections {
			...
			if msg, op, err := wsutil.ReadClientData(conn); err != nil {
				...
				if ss[0] != "" {
				   	err = wsutil.WriteServerMessage(epoller.connections[epoller.token[ss[0]]], op, msg)
					if err != nil {
						// handle error
					} 
				} 
...
func (e *epoll) Remove(conn net.Conn) error {
	...
	e.lock.Lock()
	defer e.lock.Unlock() 
	delete(e.token, e.tokeninv[fd]) 
	delete(e.tokeninv, fd) 
