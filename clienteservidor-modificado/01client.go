...
`)
	...
	u := url.URL{Scheme: "ws", Host: *ip + ":5001", Path: "/?token=a"}
	log.Printf("Connecting to %s", u.String())
	var conns []*websocket.Conn
	for i := 0; i < *connections; i++ {
		c, _, err := websocket.DefaultDialer.Dial(u.String()+strconv.Itoa(i), nil)
		if err != nil {
	...
	for {
		for i := 0; i < len(conns); i++ {
			...
			conn.WriteMessage(websocket.TextMessage, []byte(fmt.Sprintf("22222222|hyyyyyyyyyyy  %v",i )))
		}
	}
}
