...
func main() {
	...
	if err := http.ListenAndServe("0.0.0.0:5005", nil); err != nil {
		log.Fatal(err)
	}
}
...
func Start() {
	for {
		...
		for tokfd, conn := range connections {
			if conn == nil {
				break
			}
			if msg, op, err := wsutil.ReadClientData(conn); err != nil {
				if err := epoller.Remove(conn); err != nil {
					...
					delete(epoller.token,epoller.tokenfd[tokfd]) 
					delete(epoller.tokenfd,tokfd) 
...
func (e *epoll) Wait() (map[int]net.Conn, error) {
	...
	for i := 0; i < n; i++ {
		...
		connections[int(events[i].Fd)] = e.connections[int(events[i].Fd)]
	} 
